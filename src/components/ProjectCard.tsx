...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <li
      data-reveal
      data-cursor-target
      className="flex w-[min(86vw,38rem)] shrink-0 snap-start flex-col gap-5"
    >
      ...
      <div>
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
          {/* Sized against the hero's own h1 (text-3xl/4xl), not against the
              card: pushed larger than this the card titles start competing
              with the page's primary heading and the hierarchy flattens. The
              track gets its extra height from the wider frames and the row's
              vertical padding instead. */}
          <h3 className="text-lg font-medium tracking-tight text-ink">{project.org}</h3>
